...
contract SupplyChain is Ownable, ConsumerRole, RetailerRole, FarmerRole, DistributorRole {
  ...
  modifier verifyCaller (address _address) {
    require(msg.sender == _address, "Not the owner of the current UPC"); 
    _;
  }

  // Define a modifier that checks if the paid amount is sufficient to cover the price
  modifier paidEnough(uint _price) { 
    require(msg.value >= _price, "Insufficient fund provided"); 
    _;
  }
  ...
  FarmerRole.onlyFarmer
  {
    // Add the new item as part of Harvest
  
  items[_upc] = Item(
    sku,  // Stock Keeping Unit (SKU)
    _upc, // Universal Product Code (UPC), generated by the Farmer, goes on the package, can be verified by the Consumer
    msg.sender,  // Metamask-Ethereum address of the current owner as the product moves through 8 stages
    ...
    payable(address(0)) // Metamask-Ethereum address of the Consumer
  );
  ...
  function buyItem(uint _upc) public payable 
    ...
    checkValue(_upc)
    DistributorRole.onlyDistributor
    {
  ...
  function fetchItem(uint _upc) public view returns (Item memory item){
    return items[_upc];
  }


  // Define a function 'fetchItemBufferOne' that fetches the data
  // Original implementation with long list of retuen variables was giving an error
